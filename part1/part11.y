...
typedef struct {
    char* place;   // temp variable like "t0"
    char* code;    // generated 3AC code
} result;
char* new_temp();
char* new_label();
result* create_result(char* place, char* code);
...
result* create_result(char* place, char* code) {
    ...
    r->place = place ? strdup(place) : NULL;
